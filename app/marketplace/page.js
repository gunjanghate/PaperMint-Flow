...
export default function Datasets() {
  ...
  const handleAccess = async (dataset) => {
    ...
    try {
      ...
      setToast({ type: 'success', message: `Paid ${priceInFLOW.toFixed(4)} FLOW!` });
      setTimeout(() => setToast(null), 4000);

      // Final confirmation toast to signal end-to-end success
      setTimeout(() => {
        const finalMsg = `Success! Payment confirmed${purchaserTokenId ? ` · NFT #${purchaserTokenId}` : ''}. Access granted.`;
        setToast({ type: 'success', message: finalMsg });
